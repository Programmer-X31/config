...
myFont :: String
myFont = "xft:Liga SFMono Nerd Font:regular:size=9:antialias=true:hinting=true"
...
myStartupHook = do
  ...
  spawnOnce "cbatticon &"
  spawnOnce "/usr/lib/xfce4/notifyd/xfce4-notifyd &" -- Notification Daemon
...
gsCategories =
  ...
  , ("Favorites",  "xdotool key super+alt+9")
  ]
...
myShowWNameTheme = def
  { swn_font              = "xft:Mononoki Nerd Font Mono:bold:size=100"
  , swn_fade              = 1.0
  , swn_bgcolor           = "#1c1f24"
  , swn_color             = "#ffffff"
  }
...
-- myWorkspaces = [" DEV ", " WWW ", " SYS ", " MUS ", " DOC "]
-- myWorkspaces = [" \xe796 ", " \xeb99 ", "  ", " \xf001 ", " \xdb86\xde98 ", "  ", "  \xdb83\xdede ", " \xead9 ", "  \xf03e "]
myWorkspaces =
         " \xe795 "   :
         " \xf268 "   :
         " \xf1bc "   :
...
myManageHook = composeAll
  ...
  , className =? "BeatboxerGui"    --> doFloat
...
myKeys c =
  ...
  , ("M-v", addName "Visual Studio Code"     $ spawn (myEditor_two))]
  ...
  [ ("M-M1-<Return>", addName "Select favorite apps" $ spawnSelected'
       $ gsInternet ++ gsMultimedia ++ gsOffice ++ gsSettings ++ gsSystem ++ gsUtilities ++ gsFavorites)
  ...
  , ("M-M1-9", addName "Menu of favorite apps"     $ spawnSelected' gsFavorites)]

  -- Monitor Mangement
  ^++^ subKeys "Monitors"
  [ ("M-.", addName "Switch focus to next monitor" $ nextScreen)
  , ("M-,", addName "Switch focus to prev monitor" $ prevScreen)
  , ("M-s 1", addName "Single HDMI-2 Monitor"      $ spawn "xrandr --output eDP-1 --off --output DP-1 --off --output HDMI-1 --off --output DP-2 --off --output HDMI-2 --mode 1600x900 --pos 0x0 --rotate normal")
  , ("M-s 2", addName "Single Laptop Monitor"      $ spawn "xrandr --output eDP-1 --primary --mode 1366x768 --pos 0x0 --rotate normal --output DP-1 --off --output HDMI-1 --off --output DP-2 --off --output HDMI-2 --off")
  , ("M-s 3", addName "Dual Monitors"              $ spawn "xrandr --output eDP-1 --primary --mode 1366x768 --pos 1600x0 --rotate normal --output DP-1 --off --output HDMI-1 --off --output DP-2 --off --output HDMI-2 --mode 1600x900 --pos 0x0 --rotate normal")]
  ...
  , ("M-e w", addName "Emacsclient EWW Browser"  $ spawn (myEmacs ++ ("--eval '(doom/window-maximize-buffer(eww \"google.com\"))'")))]
  ...
  , ("<XF86HomePage>", addName "Open home page"       $ spawn (myBrowser ++ " https://www.youtube.com/"))
  , ("<XF86Mail>", addName "Email client"             $ spawn myMailClient)
...
main = do
  ...
  xmproc0 <- spawnPipe ("xmobar -x 0 $HOME/.config/xmobar/default-xmobarrc")
  --xmproc1 <- spawnPipe ("xmobar -x 0 $HOME/.config/xmobar/default-xmobarrc-2")
